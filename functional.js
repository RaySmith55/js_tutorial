...
function urlCreation(string) {
    return "https://example.com/" + `${urlify(string)}`;
}

function urlMap(states) {
    return states.map(state => urlCreation(state));
}
...
function includesDakotaFilter(states) {
    return states.filter(state => state.includes("Dakota"));
}
console.log(includesDakotaFilter(states));

function twoLengthFilter(states) {
    return states.filter(state => state.split(/\s+/).length === 2);
}
console.log(twoLengthFilter(states));
